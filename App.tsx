  ...
  return (
    <div className={`flex flex-col h-screen max-w-md mx-auto overflow-hidden shadow-2xl relative font-sans transition-colors duration-300 ${bgMain}`}>
      ...
      <main className="flex-1 relative overflow-hidden flex flex-col">
                        ...
                        {stats.categories.map((cat) => {
                          ...
                          return (
                            <div key={cat.category} className={`${bgCard} rounded-[2rem] shadow-sm border ${borderCard} overflow-hidden`}>
                               <div 
                                 className={`p-5 flex items-center justify-between cursor-pointer ${isDark ? 'active:bg-white/5' : 'active:bg-gray-50'}`}
                                 ...
                                 <div className={`px-5 pb-5 pt-0 space-y-2`}>
                                   {categoryTransactions.map(t => (
                                     <div key={t.id} className={`${isDark ? 'bg-white/5' : 'bg-[#F9F9FB]'} p-4 rounded-[1.2rem] flex items-center justify-between border ${isDark ? 'border-white/5' : 'border-gray-50'}`}>
                                       <div>
                                         <p className={`text-xs font-bold ${textPrimary}`}>{t.note}</p>
                                         <p className={`text-[9px] ${isDark ? 'text-[#8E8E93]' : 'text-gray-400'} mt-1`}>{t.date.toLocaleDateString()}</p>
                                       </div>
        ...
        {activeTab === 'budget' && (
          <div className="h-full overflow-y-auto px-4 pb-32 space-y-6 no-scrollbar pt-8">
             <div className="px-2">
                <h2 className={`text-2xl font-extrabold ${textPrimary}`}>Budget Control</h2>
                <p className={`text-sm ${textSecondary} font-medium`}>Monthly limits and performance.</p>
             </div>

             <div className={`${bgCard} p-6 rounded-[2.5rem] shadow-sm border ${borderCard} flex flex-col items-center text-center space-y-4 relative`}>
                ...
                <div><p className={`text-[10px] font-bold ${isDark ? 'text-[#8E8E93]' : 'text-gray-600'} uppercase tracking-widest`}>Global Monthly Goal</p><p className={`text-3xl font-black ${textPrimary}`}>{formatCurrency(globalBudgetLimit)}</p></div>
                ...
                <div className="flex justify-between w-full text-[11px] font-bold"><span className={isDark ? 'text-[#8E8E93]' : 'text-gray-600'}>Spent: {formatCurrency(stats.totalSpent)}</span><span className="text-blue-700">Budget: {formatCurrency(globalBudgetLimit)}</span></div>
             </div>

             <section className="space-y-3 pb-10">
                <div className="flex justify-between items-end px-2">
                  <h3 className={`text-lg font-bold ${textPrimary}`}>Category Budgets</h3>
                  {activeBudgets.length > 0 && (
                    <button onClick={() => setIsBudgetEditorOpen(true)} className="text-xs font-bold text-blue-700">Edit</button>
                  )}
                </div>
                {activeBudgets.length > 0 ? (
                  activeBudgets.map((b) => {
                    ...
                    return (
                      <div key={b.category} className={`${bgCard} p-4 rounded-[1.5rem] shadow-sm border ${borderCard} flex items-center justify-between group transition-all`}>
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0 pr-2">
                                <p className={`text-sm font-bold ${textPrimary} truncate mb-1`}>{b.category}</p>
                                <div className="flex items-center gap-3 w-[120px]">
                                  <div className={`flex-1 ${isDark ? 'bg-[#2C2C2E]' : 'bg-gray-100'} h-1.5 rounded-full overflow-hidden`}>
                                  ...
                                  <span className={`text-[10px] font-bold whitespace-nowrap ${isOver ? 'text-red-600' : (isDark ? 'text-[#8E8E93]' : 'text-gray-500')}`}>{percent}%</span>
                                </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-3 shrink-0">
                            <div className="text-right mr-1"><p className={`text-sm font-bold ${isOver ? 'text-red-500' : textPrimary}`}>{formatCurrency(spent)}</p><p className={`text-[10px] ${isDark ? 'text-[#8E8E93]' : 'text-gray-400'} font-bold`}>Goal: {formatCurrency(b.limit)}</p></div>
                  ...
                  <div className={`text-center py-12 px-6 ${bgCard} rounded-[2rem] border border-dashed ${isDark ? 'border-[#2C2C2E]' : 'border-gray-200'} mx-2`}><Target className="mx-auto text-gray-300 mb-3" size={32} /><p className={`text-sm font-bold ${isDark ? 'text-[#8E8E93]' : 'text-gray-500'} mb-1`}>Enter Your Budgets</p><p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'} mb-6`}>Manually set your spending goals to see tracking bars here.</p><button onClick={() => setIsBudgetEditorOpen(true)} className="py-2.5 px-6 bg-blue-700 text-white rounded-xl text-xs font-bold active:scale-95 transition-all shadow-md flex items-center gap-2 mx-auto"><Edit2 size={14} /> Set Limits</button></div>
                )}
             </section>
          </div>
        )}
        ...
        {activeTab === 'subscriptions' && (
          <div className="h-full overflow-y-auto px-4 pb-32 space-y-6 no-scrollbar pt-8">
             <div className="px-2">
                <h2 className={`text-2xl font-extrabold ${textPrimary}`}>Subscriptions</h2>
                <p className={`text-sm ${textSecondary} font-medium`}>Manage your repeating payments.</p>
             </div>
             ...
             <div className={`${bgCard} p-6 rounded-[2.5rem] shadow-sm border ${borderCard} flex items-center justify-between gap-4`}>
                ...
                <div className="flex-1">
                  <p className={`text-[10px] font-bold ${isDark ? 'text-[#8E8E93]' : 'text-gray-500'} uppercase tracking-widest`}>Total Monthly Cost</p>
                  <p className={`text-2xl font-black ${textPrimary}`}>{formatCurrency(totalMonthlySubCost)}</p>
                </div>
               ...
               <div className="flex overflow-x-auto gap-4 no-scrollbar pb-2 px-2">
                 {QUICK_ADD_SUBSCRIPTIONS.map((service) => (
                   <button key={service.name} onClick={() => quickAddSub(service)} className="flex flex-col items-center gap-2 shrink-0 group active:scale-95 transition-transform">
                     <div className={`w-16 h-16 ${bgCard} rounded-2xl shadow-sm border ${borderCard} flex items-center justify-center overflow-hidden group-hover:shadow-md transition-shadow`}>
             ...
             <section className="space-y-3 pb-10">
                ...
                <div className="space-y-3">
                  {subscriptions.length > 0 ? (
                    subscriptions.map((s) => (
                      <div key={s.id} className={`${bgCard} p-4 rounded-[1.5rem] shadow-sm border ${borderCard} flex items-center justify-between group`}>
                          <div className="flex items-center gap-4 flex-1">
                            <div className={`w-12 h-12 rounded-2xl ${isDark ? 'bg-[#2C2C2E]' : 'bg-gray-50'} flex items-center justify-center overflow-hidden border ${borderCard}`}>
                              {s.iconUrl ? <img src={s.iconUrl} alt={s.name} className="w-8 h-8 object-contain" /> : <div className={`w-full h-full flex items-center justify-center text-xs font-black ${isDark ? 'bg-blue-500/20' : 'bg-blue-100'} text-blue-700 uppercase`}>{s.name.slice(0,2)}</div>}
                            </div>
                            <div>
                                <p className={`text-sm font-bold ${textPrimary}`}>{s.name}</p>
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className={`text-[10px] font-bold ${isDark ? 'text-[#8E8E93]' : 'text-gray-400'} flex items-center gap-1`}><Clock size={10} /> Next: {new Date(s.nextBillingDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                                  <span className={`text-[10px] font-black ${isDark ? 'text-blue-400 bg-blue-400/10' : 'text-blue-600 bg-blue-50'} uppercase px-1.5 py-0.5 rounded`}>{s.billingCycle}</span>
                                </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="text-right mr-1">
                                <p className={`text-sm font-black ${textPrimary}`}>{formatCurrency(s.amount)}</p>
                            ...
                            <button onClick={() => deleteSub(s.id)} className={`p-2 ${isDark ? 'text-[#3A3A3C]' : 'text-gray-200'} hover:text-red-500 active:scale-90 transition-all`}><Trash2 size={16} /></button>
                          </div>
                      </div>
                    ))
                  ) : (
                    <div className={`text-center py-16 ${bgCard} rounded-[2.5rem] border border-dashed ${borderCard} mx-2`}><AlertCircle className="mx-auto text-gray-200 mb-4" size={48} /><p className={`text-sm font-bold ${isDark ? 'text-[#8E8E93]' : 'text-gray-400'}`}>No active subscriptions</p></div>
                  )}
                </div>
             </section>
          </div>
      ...
      <nav className={`fixed bottom-0 left-0 right-0 max-w-md mx-auto ${isDark ? 'bg-[#1C1C1E]/90' : 'bg-white/90'} backdrop-blur-xl border-t ${isDark ? 'border-[#2C2C2E]' : 'border-gray-100'} safe-area-bottom pb-4 pt-3 flex justify-around items-center z-50 transition-colors`}>
        <button onClick={() => setActiveTab('overview')} className={`flex flex-col items-center gap-1 flex-1 ${activeTab === 'overview' ? 'text-blue-500' : 'text-gray-500'}`}><BarChart3 size={24} strokeWidth={activeTab === 'overview' ? 2.5 : 2} /><span className="text-[10px] font-bold">Overview</span></button>
        <button onClick={() => setActiveTab('budget')} className={`flex flex-col items-center gap-1 flex-1 ${activeTab === 'budget' ? 'text-blue-500' : 'text-gray-500'}`}><Calendar size={24} strokeWidth={activeTab === 'budget' ? 2.5 : 2} /><span className="text-[10px] font-bold">Budget</span></button>
        <div className="flex-1 flex justify-center"><button onClick={() => setIsAddModalOpen(true)} className="w-12 h-12 bg-blue-700 text-white rounded-full flex items-center justify-center shadow-lg active:scale-90 transition-transform -mt-6 border-4 border-white dark:border-[#1C1C1E]"><Plus size={28} strokeWidth={2.5} /></button></div>
        <button onClick={() => setActiveTab('subscriptions')} className={`flex flex-col items-center gap-1 flex-1 ${activeTab === 'subscriptions' ? 'text-blue-500' : 'text-gray-500'}`}><Wallet size={24} strokeWidth={activeTab === 'subscriptions' ? 2.5 : 2} /><span className="text-[10px] font-bold">Subscriptions</span></button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
          ...
          <div className={`relative ${bgCard} w-full max-w-xs rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in duration-200`}>
            <div className="p-6 text-center">
              <h3 className={`text-lg font-bold ${textPrimary}`}>{confirmModal.title}</h3>
              <p className={`text-sm ${textSecondary} mt-2 font-medium`}>{confirmModal.message}</p>
            </div>
      ...
      {isBudgetEditorOpen && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center px-4 pb-8 sm:px-0">
          ...
          <div className={`relative w-full max-w-md ${bgCard} rounded-t-[2.5rem] shadow-2xl p-6 animate-in slide-in-from-bottom duration-300 max-h-[85vh] flex flex-col transition-colors`}>
            <div className="flex justify-between items-center mb-6 shrink-0">
              <div className="flex items-center gap-2"><Target size={20} className="text-blue-500" /><h2 className={`text-2xl font-extrabold ${textPrimary}`}>Set Limits</h2></div>
              <button onClick={() => setIsBudgetEditorOpen(false)} className={`w-8 h-8 ${isDark ? 'bg-white/10' : 'bg-gray-100'} rounded-full flex items-center justify-center ${isDark ? 'text-gray-400' : 'text-gray-400'}`}><X size={18} /></button>
            </div>
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pb-6">
              {expenseCategories.map(cat => {
                ...
                return (
                  <div key={cat} className={`${bgCard} rounded-2xl p-4 flex items-center justify-between border-2 ${isDark ? 'border-white/5' : 'border-gray-50'}`}>
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <span className={`text-sm font-bold truncate ${textPrimary}`}>{cat}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className={`text-[10px] font-bold ${isDark ? 'text-gray-500' : 'text-gray-300'}`}>{CURRENCY_SYMBOLS[currency]}</span>
                      <input 
                        type="number" value={limitValue === 0 ? '' : limitValue} 
                        onChange={(e) => handleUpdateBudget(cat, parseFloat(e.target.value) || 0)} 
                        placeholder="0" className={`w-24 ${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-xl py-2 px-3 text-sm font-bold ${textPrimary} outline-none text-right`} 
                      />
                    </div>
                  </div>
                );
        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center px-4 pb-8 sm:px-0">
          ...
          <div className={`relative w-full max-w-md ${bgCard} rounded-t-[2.5rem] shadow-2xl p-6 animate-in slide-in-from-bottom duration-300 max-h-[90vh] flex flex-col transition-colors`}>
            ...
            <form onSubmit={handleAddSubscription} className="space-y-6 flex-1 overflow-y-auto no-scrollbar pb-6 px-1">
              <div className="space-y-2">
                <label className={`text-[10px] font-black ${isDark ? 'text-[#8E8E93]' : 'text-gray-400'} uppercase tracking-widest px-2`}>Service Name</label>
                <input autoFocus type="text" value={subForm.name} onChange={(e) => setSubForm({...subForm, name: e.target.value})} placeholder="Netflix, Spotify..." className={`w-full ${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-2xl py-4 px-4 text-sm font-bold outline-none ${textPrimary}`} />
              </div>
              <div className="space-y-2">
                <label className={`text-[10px] font-black ${isDark ? 'text-[#8E8E93]' : 'text-gray-400'} uppercase tracking-widest px-2`}>Price & Cycle</label>
                <div className="flex gap-2">
                  <input type="number" step="0.01" value={subForm.amount || ''} onChange={(e) => setSubForm({...subForm, amount: parseFloat(e.target.value) || 0})} placeholder="0.00" className={`flex-1 ${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-2xl py-4 px-4 text-xl font-extrabold outline-none ${textPrimary}`} />
                  <select value={subForm.billingCycle} onChange={(e) => setSubForm({...subForm, billingCycle: e.target.value as BillingCycle})} className={`${isDark ? 'bg-[#2C2C2E] text-white' : 'bg-gray-50 text-black'} rounded-2xl px-4 font-bold text-sm outline-none`}><option value={BillingCycle.Monthly}>Monthly</option><option value={BillingCycle.Yearly}>Yearly</option></select>
                </div>
              </div>
              <div className="space-y-2">
                <label className={`text-[10px] font-black ${isDark ? 'text-[#8E8E93]' : 'text-gray-400'} uppercase tracking-widest px-2`}>Next Billing Date</label>
                <input type="date" value={subForm.nextBillingDate} onChange={(e) => setSubForm({...subForm, nextBillingDate: e.target.value})} className={`w-full ${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-2xl py-4 px-4 text-sm font-bold outline-none ${textPrimary}`} />
              </div>
              <div className="space-y-3">
        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center px-4 pb-8 sm:px-0">
          ...
          <div className={`relative w-full max-w-md ${bgCard} rounded-t-[2.5rem] shadow-2xl p-6 animate-in slide-in-from-bottom duration-300 max-h-[90vh] overflow-y-auto no-scrollbar transition-colors`}>
            ...
            <form onSubmit={handleAddTransaction} className="space-y-6">
              ...
              <div className="relative"><span className={`absolute left-4 top-1/2 -translate-y-1/2 text-3xl font-bold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{CURRENCY_SYMBOLS[currency]}</span><input autoFocus type="number" step="0.01" value={newAmount} onChange={(e) => setNewAmount(e.target.value)} placeholder="0.00" className={`w-full ${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-2xl py-6 pl-12 pr-4 text-4xl font-extrabold outline-none ${textPrimary}`} /></div>
                ...
                <div className="space-y-2 text-center">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {CATEGORY_TAGS[newCategory]?.map((tag) => (
                      <button key={tag} type="button" onClick={() => setNewNote(tag)} className={`px-3 py-1.5 rounded-full text-[10px] font-bold transition-all border ${newNote === tag ? 'bg-blue-700 text-white border-blue-700' : (isDark ? 'bg-white/5 text-[#8E8E93] border-white/5' : 'bg-gray-50 text-gray-600 border-gray-100')}`}>{tag}</button>
                    ))}
                  </div>
                </div>
              </div>

              <input type="text" value={newNote} onChange={(e) => setNewNote(e.target.value)} placeholder="Custom note..." className={`w-full ${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-2xl py-4 px-4 text-sm font-bold outline-none ${textPrimary}`} />
